...
in {
	...
	helper.has_category = {
		code =
			''awk -F'\t' -v category="$1" -f ''
			+ toFile "sfeed_has_category.awk" ''
			  {
			    split($9, cs, "|")
			    for (i in cs)
			      if (cs[i] == category) {
	...
	helper.title_starts_with = {
		code =
			''awk -F'\t' -v titlestart="$1" -f ''
			+ toFile "sfeed_title_starts_with.awk" ''
			  {
	...
	helper.title_doesnt_start_with = {
		code =
			''awk -F'\t' -v titlestart="$1" -f ''
			+ toFile "sfeed_title_doesnt_start_with.awk" ''
			  {
			    if (index($2,titlestart) != 1)
			      print
			  }
	...
	helper.normalize_dates = {
		code =
			''awk -F'\t' -v OFS=$'\t' -f ''
			+ pkgs.writeText "sfeed_process_dates.awk" ''
			  BEGIN {
	...
	helper.force_increasing_dates = {
		code =
			''awk -F'\t' -v OFS=$'\t' -f ''
			+ toFile "sfeed_force_decreasing_dates.awk" ''
			  BEGIN {
			    prev = 0
			  }
			  {
