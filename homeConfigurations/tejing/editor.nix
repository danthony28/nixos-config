{
	...
	xdg.dataFile."myeditor".source =
		my.lib.mkShellScript "myeditor"
		{
			inputs = [pkgs.emacs];
	...
	xsession.windowManager.i3.config.assigns."10" = [
		{
			class = "^Emacs$";
			instance = "^emacsOrgAgenda$";
		}
	];
	xsession.windowManager.i3.config.startup = [
		{
			command = "${my.launch} app emacs-org-agenda ${pkgs.writeShellScript "emacs-org-agenda-cycle" ''while true; do emacsclient -cF '((name . "emacsOrgAgenda"))' --eval '(org-agenda "" "n")';done''}";
			always = false;
			notification = false;
		}
	];
