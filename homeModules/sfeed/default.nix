{
	...
	feeds_function =
		defineFunctions {
			feeds =
				if length (attrValues cfg.rc.feeds) == 0
				then ":"
					...
					++ [
						{
							inherit name;
							value =
								caseStatement ''"$1"'' ''_${name} "$@" || return''
								(mapAttrs
									(_: v:
											if ! hasContext v."${name}" && hasAttr v."${name}" cfg.rc."${name}"
		...
		resholve.writeScript "sfeedrc"
		{
			...
			inputs =
				unique ([pkgs.gawk]
					...
					(concatMap attrValues (attrValues cfg.rc)));
			execer =
				unique (concatMap
					(x:
							if x ? execer
